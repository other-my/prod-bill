<template>
    <div class='order-container-table' style="display: flex; flex-wrap: nowrap;justify-content: center">
        ...
        <llTable style="display: inline" key="col2" width="400px" :table-data="col2llTableDataRef">
            ...
            <template v-slot:col_7="{item}">
                <tableUnit :title="item.title" :span-rows="2" :unit-data="item.unitData"></tableUnit>
            </template>
        </llTable>
        <llTable style="display: inline" key="col3" width="400px" :table-data="col3llTableDataRef">
            ...
            <template v-slot:col_3="{item}">
                <tableUnit :title="item.title" :span-rows="24" :unit-data="item.unitData">
                    <template v-slot:item_whole="{item:item1}">
                        <div style="width: 100%; height: 100%; display: flex; flex-wrap: wrap; margin: 10px">
                            <template v-for="img in item1.enums">
                                <img :src="img" alt="">
                            </template>
                        </div>
                    </template>
    ...
    <div style="position: absolute;right: 300px; bottom: 20px">
        <el-button plain @click="saveChange">保存</el-button>
    </div>
</template>
<script>
